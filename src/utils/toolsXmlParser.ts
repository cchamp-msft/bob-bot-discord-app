...
function parseToolElement(raw: Record<string, unknown>, index: number): ToolConfig {
  ...
  const api = raw.api;
  // NOTE: keep in sync with VALID_TOOL_APIS in config.ts (tested by toolsXmlParser.test.ts)
  const validApis = ['comfyui', 'ollama', 'accuweather', 'nfl', 'serpapi', 'meme', 'discord', 'xai', 'xai-image', 'xai-video', 'webfetch'];
